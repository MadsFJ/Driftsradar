...
//
// Brug: node scripts/hent-status.mjs [output-fil]
import { readFile, writeFile } from 'node:fs/promises';
import { fileURLToPath } from 'node:url';
import path from 'node:path';

const root = path.join(path.dirname(fileURLToPath(import.meta.url)), '..');
const outFile = process.argv[2] || path.join(root, 'status.json');
...
async function fetchOne(url, svc) {
  ...
  try {
    const res = await fetch(url, {
      ...
      headers: {
        'User-Agent': 'Driftsradar/1.0 (+https://github.com/MadsFJ/Driftsradar)',
...
const data = JSON.parse(await readFile(path.join(root, 'services.json'), 'utf8'));
